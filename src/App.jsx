...
export default function KineQuestionnaire() {
  // phase: "intro" | "bodymap" | "perzone" | "common" | "end"
  const [phase, setPhase]     = useState("intro");
  const [fading, setFading]   = useState(false);

  // BodyMap results
  const [bodyMapResult, setBodyMapResult] = useState([]);

  // Zone selection (derived from bodymap)
      ...
      <div style={{
        ...
        {phase === "intro" && (
          <div style={{ textAlign:"center" }}>
            ...
            <button onClick={() => fade(() => setPhase("bodymap"))} style={{
        ...
        {/* ── BODY MAP ── */}
        {phase === "bodymap" && (
          <BodyMap onComplete={(result) => {
            setBodyMapResult(result);
            const zones = [...new Set(result.map(r => r.zone))];
            setSelZones(zones);
            fade(() => { setZoneIdx(0); setSubIdx(0); setPhase("perzone"); });
          }} />
        )}
